...
from evogym.envs import CarrySmallRect, PackageBase, PushSmallRect
...
class CustomPushSmallRect(PushSmallRect):

    def __init__(self, body, connections=None):
        # make world
        super().__init__(np.asarray([[3]]), connections)
        self.world = EvoWorld.from_json(
            pkg_resources.resource_filename('bbbqd.environments', os.path.join('envs/CustomPusher-v0.json')))
        self.world.add_from_array('robot', body, 1, 1, connections=connections)
...
register(
    id='CustomPusher-v0',
    entry_point='bbbqd.environments.extended_envs:CustomPushSmallRect',
    max_episode_steps=500
)
